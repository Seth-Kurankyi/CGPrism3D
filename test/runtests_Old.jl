...
@time tet2 = tensorGlue(dataT,dataT,[1,2,3],[1,2,3])
@time prAn = tensorGlue(tet2,dataT,[2,7,9],[1,2,3])
sp = sortperm(prAn[1])
...
@time pruvA = tensorGlue(prA,prA,[1,5,6,8,9],[2,4,6,12,11])
@time pruvAA = tensorSum(pruvA,[1])
...
@time f133A = fullSplit3D(fA012F,[9,6,18],[1,2,3,4,5,10,12,13,14,15,16,17],[7,8,11])
...
@time CGPrsmA1 = fullSplit3D(f133A[2],[8,10,2],[1,3,5,6,7,9,12,14,15],[13,4,11])
...
@show length(CGPrsmA1[1][2]),length(CGPrsmA1[2][2])
